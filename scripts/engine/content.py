...
def _get_or_create_citizen_voices_issue() -> int:
    run(["gh", "label", "create", "citizen-voices", "--repo", REPO,
         "--color", "d93f0b", "--description", "Daily citizen diary", "--force"])
    issues = gh_json([
    ...
    tmp.write_text(
        "Daily diary entries from fictional Gitizens citizens, "
        "updated every day by the world engine.",
        encoding="utf-8",
    )
    result = run(["gh", "issue", "create", "--repo", REPO,
                  "--title", "[Citizen Voices] Daily Diary",
